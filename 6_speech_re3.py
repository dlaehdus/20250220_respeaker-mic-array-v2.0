...
def camera_thread_func(quit_event):
    ...
    try:
        while not quit_event.is_set():
            ...
            results = yolo_model.track(frame, conf=0.3, persist=True, verbose=False)  # conf 약간 올려 노이즈 줄임
                ...
                for i, box in enumerate(results[0].boxes.xyxy.cpu().numpy()):
                    ...
                    for x in range(x1, x2 + 1, step):
                        for y in range(y1, y2 + 1, step):
                            ...
                            d = depth_frame.get_distance(x_clamped, y_clamped)
                            if 0.15 < d < 3.0:
                    ...
                    side_x = "오른쪽" if p[0] > 0.05 else ("왼쪽" if p[0] < -0.05 else "가운데")
                    side_y = "위쪽" if p[1] < -0.05 else ("아래쪽" if p[1] > 0.05 else "가운데")
                    position = f"{side_y} {side_x}".strip()

                    temp_objects.append(f"{label}(위치:{position}, 거리:{depth:.3f}m)")

            with data_lock:
                current_seen_objects = list(set(temp_objects))  # 중복 제거
    ...
    while not quit_event.is_set():
        if mic.wakeup('respeaker'): 
            ...
            try:
                ...
                with data_lock:
                    vision_context = ", \n".join(current_seen_objects) if current_seen_objects else "인식된 물체 없음"
...
관련된 액션이 있다면 답변 끝에 'SELECTED_ACTION: [액션명]' 형식을 반드시 포함해."""
                ...
                print(f"\n[답변 (소요시간: {end_time-start_time:.2f}초)]")
                print(f"{response}")
                print("-" * 50)
